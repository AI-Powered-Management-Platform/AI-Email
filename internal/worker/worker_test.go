...
func TestEventIDsAreStableAcrossRetries(t *testing.T) {
	...
	_, _ = New(q1, &fakeSender{response: delivery.Response{Result: delivery.Sent}}, &fakeKeys{}, fakeSigner{}, first, newFakeSuppressor(), []byte("test-secret"), "", quietLogger(), 0).
		tick(context.Background())

	second := &fakeEvents{}
	q2 := newFakeQueue(msg("m1", 3))
	_, _ = New(q2, &fakeSender{response: delivery.Response{Result: delivery.Sent}}, &fakeKeys{}, fakeSigner{}, second, newFakeSuppressor(), []byte("test-secret"), "", quietLogger(), 0).
		tick(context.Background())
